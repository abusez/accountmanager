...
public class Events {
    ...
    public void initGuiEvent(GuiScreenEvent.InitGuiEvent.Post event) {
        ...
        if (event.gui instanceof GuiDisconnected) {
            try {
                Field f = ReflectionHelper.findField(GuiDisconnected.class, "message", "message");
    ...
    public void onClick(GuiScreenEvent.ActionPerformedEvent event) {
        if ((event.gui instanceof GuiSelectWorld || event.gui instanceof GuiMultiplayer) && event.button.id == 69) {
